...
3. debug 只保存合成数据和专属 canary 身份。真实 canary 凭据必须只属于 debug，不与生产共享 refresh owner；不得从生产整库复制账号、token、余额、日志或用户数据。
   若生产活跃 provider 无法在不复制凭据、不争用 refresh owner 的前提下建立 debug-only 身份，禁止降低隔离要求。可采用窄化闭环：精确 SHA 全量 CI + 同 SHA 合成协议/runtime 证据 + 生产切换前只读 inventory 与有意义基线 + 已证明的 image rollback + 切换后每个活跃 provider 一次受控 canary。证据必须明确区分切换前 contract 与切换后 live confirmation；迁移、数据库写路径、认证写入和无法 image-only rollback 的变更不能用此闭环替代 debug 实证。
...
3. 再执行 debug 已通过的低风险生产 canary 和日志窗检查；采用上述无共享凭据闭环时，切换后 live confirmation 是发布完成的硬条件。失败时只在已证明 image rollback 兼容时回退应用；数据库恢复、配置变更和账号处置需要单独授权。
...
- `scripts/plan-sub2api-upgrade.sh`：只读验证版本/上游基线，消费脱敏 active inventory，并按 U/M 来源、精确路径规则和生产活跃能力生成 case 级矩阵计划。
