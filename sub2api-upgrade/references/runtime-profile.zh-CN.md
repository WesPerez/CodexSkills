...
此档案记录 2026-07-17 已核验的默认部署。每次升级必须重新检查，尤其是文档与实际配置发生冲突时以实时运行态为准。
...
| Router | `127.0.0.1:13081` | `/ready` 必须为 `{"status":"ready",...}`；只查 13080 不足。 |
| 公网入口 | Nginx `wooai.cc.cd` -> Router `13081` -> Sub2API `13080` | 用本机 `--resolve wooai.cc.cd:443:127.0.0.1` 验证实际 SNI 链路。 |
...
| Debug | 文档目标为 `/root/sub2api-debug-deploy` 和 `:13082` | 2026-07-17 路径缺失；下次发布先核验或安全重建隔离环境，不能跳过。 |
