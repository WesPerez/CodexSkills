...
此文件是 2026-07-24 已核验快照：**当前生产** `61d5b363fe7cd370f73517973aec361303afb77f`（`0.1.164`）；上游基线 `cb24522dd53f8f363d008e3afdc8e4baf9788cab`；**上一生产** `13b41759f68a85d86c38fba4dad12f13b4792682`（`0.1.162` 线）。它用于缩短下一次 discovery，不能替代实时 `merge-base`、range-diff、源码、CI 和运行证据。

生产 5 个逻辑提交已在 `0.1.164` 上按职责整理为 4 个提交。下一次升级先验证这些职责仍存在；不能因 SHA、文件名或提交数变化就判定职责消失。不触碰 Router/Composite 职责，除非实时 diff 明确触及且已单独对照。
